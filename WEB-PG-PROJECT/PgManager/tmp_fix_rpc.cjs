...
async function main() {
    ...
    // Attempt to drop both versions to be sure
    try {
        await client.query("DROP FUNCTION IF EXISTS public.generate_monthly_invoices();");
        console.log("Dropped generate_monthly_invoices()");
    } catch(e) { console.error(e.message); }
    
    try {
        await client.query("DROP FUNCTION IF EXISTS public.generate_monthly_invoices(uuid);");
        console.log("Dropped generate_monthly_invoices(uuid)");
    } catch(e) { console.error(e.message); }

    // Now recreate the correctly typed one
    const query = `
...
BEGIN
    ...
    FOR r_tenant IN (
        SELECT t.id, t.full_name, t.move_in_date, t.rent_per_month, t.maintenance_amount, t.maintenance_type, t.security_deposit, r.rent as room_rent
        ...
        WHILE v_next_invoice_date <= CURRENT_DATE LOOP
            IF NOT EXISTS (
                ...
                -- Support both monthly and first-time charges
                v_maint_amount := 0;
                IF r_tenant.maintenance_type = 'monthly' THEN
                    v_maint_amount := COALESCE(r_tenant.maintenance_amount, 0);
                ELSIF r_tenant.maintenance_type = 'one_time' AND v_is_first_invoice THEN
                    v_maint_amount := COALESCE(r_tenant.maintenance_amount, 0);
                END IF;
            ...
            -- Handle missing Deposit separately if this is the start
            IF v_is_first_invoice AND COALESCE(r_tenant.security_deposit, 0) > 0 THEN
                IF NOT EXISTS (SELECT 1 FROM public.invoices WHERE tenant_id = r_tenant.id AND type = 'DEPOSIT') THEN
                    INSERT INTO public.invoices (
                        tenant_id, owner_id, billing_period_start, billing_period_end,
                        total_amount, paid_amount, status, type
                    ) VALUES (
                        r_tenant.id, p_owner_id, v_next_invoice_date, v_next_invoice_date,
                        r_tenant.security_deposit, 0, 'UNPAID', 'DEPOSIT'
                    );
                    v_invoices_created := v_invoices_created + 1;
                END IF;
            END IF;
...
$function$;
    ...
    console.log("Successfully recreated public.generate_monthly_invoices(uuid)");
