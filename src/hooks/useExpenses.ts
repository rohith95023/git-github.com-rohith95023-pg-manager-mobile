...
export const useExpenses = (_props: UseExpensesProps) => {
  ...
  useEffect(() => {
    ...
    const loadDraft = async () => {
      try {
        const savedDraft = await AsyncStorage.getItem('pg_expense_form_draft');
        if (savedDraft) {
  ...
  // Sync formData to AsyncStorage
  useEffect(() => {
    if (Object.values(formData).some(val => val !== "" && val !== "all")) {
      AsyncStorage.setItem('pg_expense_form_draft', JSON.stringify(formData));
    }
  }, [formData]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      resetForm();
      await AsyncStorage.removeItem('pg_expense_form_draft');
    } catch (err: any) {
