...
// import { useSearchParams } from "react-router-dom"; // REMOVED for RN
...
export const usePGs = () => {
  ...
  const showToast = (message: string, type = "success") => {
    ...
    // TODO: Use Native-compatible timeout or toast library
  ...
  useEffect(() => {
    const loadDraft = async () => {
      try {
        const saved = await AsyncStorage.getItem("pg_onboarding_draft");
        if (saved) {
  ...
  useEffect(() => {
    if (showModal && !editingPg) {
      AsyncStorage.setItem("pg_onboarding_draft", JSON.stringify({
        formData,
        currentStep,
        editingPg: null
      }));
  ...
  // const [searchParams, setSearchParams] = useSearchParams(); // REMOVED for RN

  /* REMOVED: Navigation logic based on SearchParams
  useEffect(() => {
