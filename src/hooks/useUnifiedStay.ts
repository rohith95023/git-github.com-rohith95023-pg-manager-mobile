...
import { personalDetailsSchema, stayDetailsSchema } from "../schemas/stay.schema";
...
export const useUnifiedStay = (initialData: any, isOpen: boolean, onClose: () => void, onSuccess: (action: string, paid: boolean) => void) => {
    ...
    // Refactored for AsyncStorage async loading
    useEffect(() => {
        const loadDraft = async () => {
            if (isOpen && !initialData) {
                try {
                    const saved = await AsyncStorage.getItem("unifiedStayManager_draft");
                    if (saved) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (initialData) {
                ...
                // If not initialData, draft loading effect handles it
            }
            fetchPGs();
        }
    }, [isOpen, initialData]);
    ...
    useEffect(() => {
        if (isOpen && !initialData) {
            AsyncStorage.setItem("unifiedStayManager_draft", JSON.stringify(formData));
        }
    ...
    const handleInputChange = (e: any) => { // Type changed for simplicity in RN migration preparation
    ...
    const validateStep1 = () => {
        // TODO: Ensure schemas are compatible with Native (usually they are)
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            AsyncStorage.removeItem("unifiedStayManager_draft");
    ...
    return {
        ...
        toast, setToast,
        handleClose: () => { AsyncStorage.removeItem("unifiedStayManager_draft"); onClose(); }
    };
};
